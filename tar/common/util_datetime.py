...
def convertBaseTimestamp(timestamp, periodPerSec):
	if periodPerSec%ONE_DAY == 0:
		base_timestamp = timestamp - (timestamp % periodPerSec) - ONE_HOUR + 9
	else:
		base_timestamp = timestamp - (timestamp % periodPerSec)
