...
async def process_node_c(request: WorkflowRequest):
    print(f"# Node C - {request}")
    # await asyncio.sleep(1.5)  # 비즈니스 로직 시뮬레이션
...
async def process_node_d(request: WorkflowRequest):
    print(f"# Node D - {request}")
    # await asyncio.sleep(0.5)  # 비즈니스 로직 시뮬레이션
...
# 각 노드별 서버 실행을 위한 설정
def run_node_a():
    uvicorn.run(app_node_a, host="127.0.0.1", port=8001)


def run_node_b():
    uvicorn.run(app_node_b, host="127.0.0.1", port=8002)


def run_node_c():
    uvicorn.run(app_node_c, host="127.0.0.1", port=8003)


def run_node_d():
    uvicorn.run(app_node_d, host="127.0.0.1", port=8004)


def run_node_e():
    uvicorn.run(app_node_e, host="127.0.0.1", port=8005)


def run_node_f():
    uvicorn.run(app_node_f, host="127.0.0.1", port=8006)


def run_node_g():
    uvicorn.run(app_node_j, host="127.0.0.1", port=8007)


def run_node_h():
    uvicorn.run(app_node_h, host="127.0.0.1", port=8008)


def run_node_i():
    uvicorn.run(app_node_i, host="127.0.0.1", port=8009)


def run_node_j():
    uvicorn.run(app_node_j, host="127.0.0.1", port=8010)


def run_node_z():
    uvicorn.run(app_node_z, host="127.0.0.1", port=8011)
...
async def test_workflow():
    async with aiohttp.ClientSession() as session:
        ...
        # Node A (Start)
        response_a = await call_api("http://localhost:8001/workflow/start", workflow_request)
        print("Node A Response:", json.dumps(response_a, indent=2))

        # Node B
        response_b = await call_api("http://localhost:8002/process", {
            **workflow_request,
        ...
        tasks = [
            call_api("http://localhost:8003/process", {
                **workflow_request,
                "input_data": response_b["data"]
            }),
            call_api("http://localhost:8004/process", {
                **workflow_request,
                "input_data": response_b["data"]
            })
        ]
        ...
        # Node E (Aggregation)
        response_e = await call_api("http://localhost:8005/aggregate", {
            **workflow_request,
            "input_data": {
