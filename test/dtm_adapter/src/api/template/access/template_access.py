...
from api.template.access.query_pool import *


class TemplateAccess(Access):
    ...
    def get_template(self, tpl_id):
        query_string = get_template_query(tpl_id)
        template = self.execute_get_one(query_string)
        return template

    def get_templates(self,):
        query_string = get_templates_query()
        template = self.execute_get(query_string)
        return template

    def delete_template(self, tpl_id, usr_id):
        query_string = delete_template_query(tpl_id, usr_id)
        result = self.execute_set(query_string)
        return result

    def create_template(self, tpl_id,create_usr_id,
                            update_usr_id, dply_typ_cd,
                            tpl_typ_cd, mnfst, params):
        query_string = create_template_query( mnfst, params, tpl_id,create_usr_id,
                            update_usr_id, dply_typ_cd,
                            tpl_typ_cd)
        result = self.execute_set(query_string)
        return result
    ...
    def get_deploy_type_code(self, ):
        query_string = get_deploy_type_code_query()
        template = self.execute_get(query_string)
        return template

    def get_template_type_code(self, ):
        query_string = get_template_type_code_query()
        template = self.execute_get(query_string)
        return template
