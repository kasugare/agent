...
class ResAdaptWorkflow(BaseModel):
    ...
    statusText: Annotated[str, Field(description='Response Status Text')]
...
class ResCallChainedModelService(BaseModel):
    answer: Annotated[str | None, Field(description='answer')] = None
...
class ReqFileUpload(BaseModel):
    upload_mode : Annotated[Optional[str], Field( description="업로드 방식")]
    file_urls : Annotated[Optional[str], Field( description="업로드 방식")] = None



class ResFileUpload(BaseModel):
