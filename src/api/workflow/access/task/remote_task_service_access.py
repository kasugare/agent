...
class RemoteTaskServiceAccess(RedisAccess):
    ...
    def set_init_data_access(self, task_service_map):
        self.hset(key=self._cache_key, mapping=task_service_map)

    def set_data_access(self, field, data):
        self.hset(key=self._cache_key, mapping={field: data})

    def set_mapping_data_access(self, data_map: dict):
        self.hset(key=self._cache_key, mapping=data_map)

    def get_data_access(self, field):
